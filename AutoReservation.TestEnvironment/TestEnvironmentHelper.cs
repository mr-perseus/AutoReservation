    ...
    public static class TestEnvironmentHelper
    {
        ...
        private static List<Reservation> Reservationen =>
            new List<Reservation>
            {
                new Reservation
                {
                    ReservationsNr = 1, AutoId = 1, KundeId = 1, Von = new DateTime(2020, 01, 10),
                    Bis = new DateTime(2020, 01, 20)
                },
                new Reservation
                {
                    ReservationsNr = 2, AutoId = 2, KundeId = 2, Von = new DateTime(2020, 01, 10),
                    Bis = new DateTime(2020, 01, 20)
                },
                new Reservation
                {
                    ReservationsNr = 3, AutoId = 3, KundeId = 3, Von = new DateTime(2020, 01, 10),
                    Bis = new DateTime(2020, 01, 20)
                },
                new Reservation
                {
                    ReservationsNr = 4, AutoId = 2, KundeId = 1, Von = new DateTime(2020, 05, 19),
                    Bis = new DateTime(2020, 06, 19)
                }
            };
        ...
        public static void InitializeTestData(this AutoReservationContext context)
        {
            ...
            try
            {
                ...
                if (luxusklasseAutoTableName != autoTableName) context.DeleteAllRecords(luxusklasseAutoTableName);
                if (mittelklasseAutoTableName != autoTableName) context.DeleteAllRecords(mittelklasseAutoTableName);
                if (standardAutoTableName != autoTableName) context.DeleteAllRecords(standardAutoTableName);
                context.DeleteAllRecords(autoTableName);
                context.DeleteAllRecords(kundeTableName);

                SeedAuto(context, luxusklasseAutoTableName, mittelklasseAutoTableName, standardAutoTableName,
                    autoTableName);
        ...
        {
            var statement = $"DELETE FROM {table}"; // Must be a separate variable
